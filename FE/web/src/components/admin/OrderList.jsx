  ...
  return (
    <>
      <div className="admin-table">
        {orders.map((order) => {
          const isFinalStatus = ['cancelled', 'completed'].includes(order.status)
            ...
            <div className="order-row" key={order._id}>
              <button className="order-summary-button" type="button" onClick={() => setSelectedOrder(order)}>
                ...
                <span className="text-muted-shop">
                  ...
                  {order.discount?.code ? ` · Mã ${order.discount.name || order.discount.code}` : ''}
                </span>
              ...
              <select
                className={`form-select order-status order-status-select ${getStatusClass(order.status)}`}
